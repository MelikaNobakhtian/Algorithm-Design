...
public static class MeasureIt
{
    ...
    static public void MeasureMelikaS20()
    {
        int a = 1;
        int b = 3;
        timer1000.Measure("Swap_Regular", delegate
         {
             int tmp = a;
             a = b;
             b = tmp;
        ...
        timer1000.Measure("Swap_Tuple", delegate
         {
             (a, b) = (b, a);
         });

        ClassNode[] classNode = new ClassNode[100];
        for (int i = 0; i < 100; i++)
            classNode[i] = new ClassNode();

        int idx = 9;
        timer1000.Measure("Array_Class_Access", delegate
         {
             classNode[idx].X+= 1;
         });

        StructNode[] structNode = new StructNode[100];
        for (int i = 0; i < 100; i++)
            structNode[i] = new StructNode();

        timer1000.Measure("Array_Struct_Access", delegate
        {
            structNode[idx].X += 1;
        });

        string dep = null;
        timer1000.Measure("Add_string", delegate
        {
            dep += 'a';
        });
        StringBuilder useless = new StringBuilder();
        timer1000.Measure("Add_StringBuilder", delegate
        {
            useless.Append('a');
        });
